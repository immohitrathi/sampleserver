...
//getMatchList
let count = 0;
RedisCache.get("getMatchDetail_status",function(rerr, rres){
	if(rerr || rres==null) {
		RedisCache.set("getMatchDetail_status","1",60);
			...
			if(!err && typeof(res)!=='undefined') {
				...
				if(typeof(liveMatches)!=='undefined' && liveMatches.length>0) {
					for(let i=0;i<liveMatches.length;i++) {
						...
						Cricket.getMatchDetailCron(matchfile, function(merr, mres){
							if(!merr && typeof(mres)!=='undefined') {
								//we got the result, save it in redis
								RedisCache.set("getMatchDetail_"+matchfile,mres);
								count++;
								if(count>=liveMatches.length) {
									RedisCache.delete("getMatchDetail_status");
									process.exit();
								}
							} else {
