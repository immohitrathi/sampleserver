...
const apiService = ( { } ) => ( next ) => ( action ) => {
    ...
    action.CALL_API.map((action)=> {
        if(action !== null) {
            const { apiDomain, path, method = "GET", body } = action.meta;
            let url = `${ apiDomain }${ path }`;
            if(path.indexOf('http') !== -1){
                url = path;
            }

            //console.log("Inside ApiService:",url);
        ...
        Promise.all(allCalls).then( (results) =>{
            action.CALL_API.map((action,idx)=> {
                ...
                if(cb){
                    ...
                    var allChildCalls = [];
                    rs.map((childAction)=> {
                        const { apiDomain, path, method = "GET", body } = childAction.meta;
